...
void IndicatorWidget::drawTurnArrow(QPainter &p, bool left, const QColor &c, int iw, int ih) {
    ...
    const int ox = (iw - aw) / 2, oy = (ih - ah) / 2;
    QPolygon arrow;
    if (left) {
        arrow << QPoint(ox,         oy + ah/2)
              << QPoint(ox + aw/2,  oy)
              << QPoint(ox + aw/2,  oy + ah/4)
              << QPoint(ox + aw,    oy + ah/4)
              << QPoint(ox + aw,    oy + 3*ah/4)
...
void IndicatorWidget::drawHighBeam(QPainter &p, const QColor &c, int iw, int ih) {
    ...
    p.setPen(pen);
    const int beamX = cx + r + 2, right = iw - 4;
    int beamOffsets[] = {-7, -3, 2, 6};
    for (int dy : beamOffsets) p.drawLine(beamX, cy + dy, right, cy + dy);
}
...
void IndicatorWidget::drawOilPressure(QPainter &p, const QColor &c, int iw, int ih) {
    ...
    QPolygon spout;
    spout << QPoint(ox + 18, oy + 4) << QPoint(ox + 28, oy + 2)
          << QPoint(ox + 28, oy + 8) << QPoint(ox + 18, oy + 8);
    p.drawPolygon(spout);
    QPainterPath drop;
